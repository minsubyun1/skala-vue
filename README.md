...
```
docs/                              핸즈온·Code Challenge별 상세 기록, images/ 에 스크린샷
src/
  api/                             외부 API 호출
    openMeteo.js                   Open-Meteo 지오코딩 + 실시간 날씨 (Key 불필요)
    openWeatherMap.js              OpenWeatherMap Current Weather + 5 Day Forecast
    ipGeolocation.js                ipwho.is IP 위치 조회
    reverseGeocode.js               Nominatim(OpenStreetMap) 역지오코딩
  components/
    code-challenge/                Code Challenge 실습 컴포넌트 (하위 도우미 컴포넌트 포함 48개)
    handson/                       핸즈온 1~8 전용 컴포넌트 (WeatherMockup, WeatherParent, WeatherScene, WeatherMapPanel 등)
    exercise/                      핸즈온 4 이후 화면들이 같이 쓰는 컴포넌트 (BaseDashboardCard, SearchBar, WeatherCard, UnitToggler)
  composables/
    useLiveWeather.js              Open-Meteo 기반 실시간 검색 상태 관리
    useOpenWeatherMap.js           OpenWeatherMap 검색 상태 관리 (현재 날씨 + 예보 동시 호출)
  data/weatherMockData.js          핸즈온 4~8이 공유하는 8개 도시 mock 데이터
  router/
    index.js                       라우터 생성, 다크 테마 화면 진입/이탈 시 html.dark 토글
    practiceRoutes.js              라우트 목록 + 사이드바 메뉴를 한 배열로 정의 (단일 소스)
  stores/
    configStore.js                 Pinia - 온도/풍속 단위 전역 상태
    counter.js                     Pinia Code Challenge용 카운터 스토어
  utils/weatherStatusStyle.js      날씨 상태별 색상·이모지 (지도 마커, 관심 지역 카드가 공용으로 사용)
  views/                           라우트에 바로 연결되는 최상위 화면 (HomeView, WeatherDashboardView, WeatherAxiosView, WeatherHomeView 등)
