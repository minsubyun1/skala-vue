...
log('1. [setup] 컴포넌트가 메모리에 생성되었습니다. (DOM 접근 불가능)')

// 부착 (Mounting) 단계
onMounted(() => {
  log('2. [onMounted] 화면에 완벽히 부착되었습니다! (API 호출/DOM 조작 적기)')
...
onUpdated(() => {
  log(`3. [onUpdated] 데이터가 변경되어 화면을 새로 그렸습니다. (현재 count: ${count.value})`)
})
...
onUnmounted(() => {
  ...
  log('4. [onUnmounted] 컴포넌트가 소멸했습니다. 타이머 청소 완료!')
})
</script>

<template>
  <div class="child-box">
    <p>자식 컴포넌트 (count: {{ count }})</p>
    <p class="hint">onMounted에서 시작한 타이머가 3초마다 count를 자동으로 올립니다.</p>
  </div>
</template>
